...
docker build -f ./Dockerfile -t bejebeje/react:$DOCKER_TAG -t bejebeje/react:$TRAVIS_BUILD_NUMBER --build-arg FONTAWESOME_NPM_AUTH_TOKEN=$FONTAWESOME_NPM_AUTH_TOKEN IDENTITY_AUTHORITY=$IDENTITY_AUTHORITY IDENTITY_CLIENT_ID=$IDENTITY_CLIENT_ID IDENTITY_REDIRECT_URI=$IDENTITY_REDIRECT_URI IDENTITY_RESPONSE_TYPE=$IDENTITY_RESPONSE_TYPE IDENTITY_SCOPE=$IDENTITY_SCOPE IDENTITY_POST_LOGOUT_REDIRECT_URI=$IDENTITY_POST_LOGOUT_REDIRECT_URI . --no-cache
